...
export class AccountsController {
  ...
  async open(@Body() dto: OpenAccountDto) {
    const id = uuid();
    await this.commands.execute(new OpenAccountCommand(id, dto.ownerId, dto.currency, dto.initialBalance));
    return { accountId: id };
  }
  ...
  async withdraw(@Param('id') id: string, @Body() dto: AmountDto) {
    await this.commands.execute(new WithdrawCommand(id, dto.amount));
    return { accountId: id };
  }
